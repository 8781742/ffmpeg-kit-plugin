...
if (!ls) {
    ...
    if (valid.length >= 2) {
        ...
        var video1 = valid[0];
        var video2 = valid[1];
        var output = "/sdcard/Download/抖音下载/merged_dex_v3.mp4";

        logi("合并: " + video1.substring(video1.lastIndexOf('/')+1));
        logi("   + " + video2.substring(video2.lastIndexOf('/')+1));
        logi("   -> " + output);
        ...
        logi("merge 结果: " + mergeOk + " (耗时 " + elapsed + "ms)");

        if (mergeOk && file.exists(output)) {
            var sizeCmd = "ls -l '" + output + "' 2>/dev/null | awk '{print $5}'";
            var sizeStr = shell.execAgentCommand(sizeCmd);
            var outSize = sizeStr ? parseInt(sizeStr.trim()) : 0;
            logi("SUCCESS: " + output);

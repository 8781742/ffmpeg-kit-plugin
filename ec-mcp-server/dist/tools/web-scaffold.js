...
export const webScaffoldTool = {
    ...
    description: "EasyClick配套Web开发脚手架。根据需求快速生成授权系统、云控后台、代理分销系统的完整项目代码。支持Go/Gin、Node/Vue3、Python/Flask技术栈。",
    inputSchema: {
        type: "object",
        properties: {
            techStack: {
...
function generateArchitecture(type, stack) {
    const diagrams = {
        "auth-system": `
┌──────────┐    HTTP     ┌──────────────┐    SQL     ┌──────────┐
│  EC设备   │──────────▶│  ${stack}后端  │──────────▶│  数据库   │
...
function generateGoGinCode(type, features) {
    const codes = [];
...
function generateNodeCode(type, features) {
...
function generatePythonCode(type, features) {
...
function generateEcIntegration(type, features) {
    const parts = [];
    parts.push(`
// ============================================
// EC 脚本端集成代码
...
function generateDeploymentGuide(stack, db) {
    if (stack === "go-gin") {
        return `# 部署指南 (Go/Gin)
...
function getTechStackRecommendation(type) {
    if (type === "auth-system") {
...
function generateGoGinProject(name, type, db, features) {
    return {
        directories: [
            `${name}/`,
            `${name}/cmd/       # 入口`,
...
function generateNodeVueProject(name, type, db, features) {
    return {
        directories: [
            `${name}/`,
...
function generatePythonProject(name, type, db, features) {
    return {
        directories: [
            `${name}/`,
            `${name}/app.py           # 主入口`,

...
export const logParserTool = {
    name: "ec_log_parse",
    description: "读取 EasyClick 脚本运行日志，自动分析错误信息，匹配已知错误库并输出修复方案。可以指定日志文件路径，也可以自动搜索项目中的日志文件。",
    inputSchema: {
        type: "object",
        properties: {
            logPath: {
    ...
    handler: async (args) => {
                    ...
                    content: [
                        {
                            ...
                            text: JSON.stringify({
                                error: true,
                                message: "未找到EC日志文件",
                                suggestion: "请提供日志文件路径(--logPath)，或确保在EC项目目录下运行",
        ...
        // 提取错误行
        const errorLines = lastNLines.filter((line) => line.toLowerCase().includes("error") ||
        ...
        // 未匹配的错误行
        const unmatchedErrors = errorLines.filter((line) => !matchedErrors.some((m) => line.includes(m.matchedLine.split(":")[0] || "")));
        return {
            content: [
                {
                    type: "text",
                    text: JSON.stringify({
                        ...
                        unmatchedErrors: unmatchedErrors.length > 0
                        ...
                        summary: matchedErrors.length > 0

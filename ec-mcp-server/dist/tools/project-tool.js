 ...
 * 项目结构分析、平台检测、配置检查
 */
import { getProjectOverview, detectPlatform, findEcProject } from "../utils/file-utils.js";
export const projectTool = {
    name: "ec_project_info",
    description: "分析 EasyClick 项目结构，检测目标平台，读取项目配置，获取项目概览。用于了解当前EC项目的整体情况。",
    inputSchema: {
        type: "object",
        properties: {
            projectDir: {
    ...
    handler: async (args) => {
        ...
        if (action === "config" || action === "all") {
            ...
            const obfuscatorConfig = readFileSafe(projectDir + "/obfuscator.json");

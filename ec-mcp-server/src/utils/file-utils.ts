 ...
 */
export function findEcProject(startDir: string): string | null {
  let current = startDir;
  for (let i = 0; i < 10; i++) {
    if (
      fs.existsSync(path.join(current, "ec_work_config")) ||
 ...
 * 读取EC项目的日志文件
 */
export function readEcLogs(projectDir: string, maxLines = 200): string {
 ...
 * 获取EC项目结构概览
 */
export function getProjectOverview(projectDir: string): string {
  const items: string[] = [];
 ...
 * 检测项目目标平台
 */
export function detectPlatform(projectDir: string): string[] {
    ...
    try {
      ...
      for (const entry of entries) {
        if (entry.isDirectory()) {
          ...
          if (name === "android") platforms.push("android");
          if (name === "ios") platforms.push("ios");
          if (name === "harmony" || name === "harmonyos") platforms.push("harmony");
        }
      }
    } catch {
      // ignore
